...
data <- readr::read_file(paste0('demo_', '66cdcef3bf89ff2f8e8cadbd', '_', 'data1', '.txt'))
datas[['data1']] <- data

data <- readr::read_file(paste0('demo_', '66cdcef3bf89ff2f8e8cadbd', '_', 'data2', '.txt'))
datas[['data2']] <- data

ngplotPostData <- list(
  account = 'cihebi',
  ngplotApiKey = '',
  template = '66cdcef3bf89ff2f8e8cadbd',
  data = datas,  
  arg = list(
    # params 
    groupCol = 'group' ,  
